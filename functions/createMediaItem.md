...
Record an uploaded file in the library. Ownership is stamped server-side
from the session, so no user key is sent.


## Installation

```tsx
import { createMediaItem } from 'uxp/components';
```

## Signature

```tsx
function createMediaItem(context: IContextProvider, item: Partial<IMediaItem>): Promise<ExecutionResult<any>>
```
