...
Absolute URL a stored library file is served from. Built exactly like
`uploadFile` builds the URL it returns, so a record's Path and the value
emitted at upload time always agree.


## Installation

```tsx
import { mediaItemUrl } from 'uxp/components';
```

## Signature

```tsx
function mediaItemUrl(context: IContextProvider, path: string): string
```
