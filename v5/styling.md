...
1. Check if a standard UXP component exists for your use case (see [Core Components](./core-components.md))
...
**Note:** CSS class names are automatically scoped at build time using scoped CSS — no manual prefixes needed to prevent conflicts. See [Lint & Scoped CSS](./lint-and-scoped-css.md) for setup.
...
- ✅ Use the font-weight tokens (`var(--fontWeightSemibold)` etc.) — rather than numeric weights or `bold`; see [Font Weights](font-weights.md)

**DON'T:**
- ❌ Hardcode colors - **always** use CSS variables
- ❌ Hardcode font weights (`600`, `bold`) - use the `--fontWeight*` tokens ([Font Weights](font-weights.md))
...
- [Lint & Scoped CSS](./lint-and-scoped-css.md) - Automatic style isolation and code quality tools
